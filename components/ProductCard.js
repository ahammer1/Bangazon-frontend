...
function ProductCard({ productObj, onUpdate }) {
  ...
  return (
    <Card style={{ width: '18rem', margin: '10px' }}>
      <Card.Img variant="top" src={productObj.image} alt={productObj.title} style={{ height: '400px' }} />
      <Card.Body>
        <Card.Title>{productObj.title}</Card.Title>
        <p className="card-text bold">{productObj.sale && <span>SALE<br /></span> } ${productObj.price}</p>
